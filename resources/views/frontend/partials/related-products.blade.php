...
<div class="related-products-section py-5">
    <div class="container">
        ...
        @if (count($products) > 1)
            <div class="owl-carousel owl-theme mechanical-carousel">
                            ...
                            <div class="product-card">
                                <div class="product-card-image">
                                    ...
                                    <div class="product-card-actions">
                                        <button onclick="addToCart('{{ $product->slug }}', 1)" class="action-btn"
                                            title="Add to Cart">
                                            <i class="fas fa-shopping-cart"></i>
                                        </button>
                                        <button onclick="quickView('{{ $product->slug }}')" class="action-btn"
                                            title="Quick View">
                                            <i class="fas fa-eye"></i>
                                        </button>
                                        <button onclick="addToWishlist('{{ $product->slug }}')" class="action-btn"
                                ...
                                <div class="product-card-content">
                                    ...
                                    <div class="product-card-price">
                                        ...
                                        @if ($product->discount > 0)
                                            <span class="current-price">Rs.
                                                {{ number_format($productAfterDiscount) }}</span>
                                    ...
                                    <div class="row px-2">
                                        <form action="{{ route('checkout', [$product->slug]) }}" method="post">
                                            @csrf
                                            <input type="hidden" name="slug" value="{{ $product->slug }}">
                                            <button type="submit" class="btn btn-cod w-100">
                                                <i class="fas fa-shopping-bag me-1"></i> Buy Now (COD)
                                            </button>
                                        </form>
                                    </div>
                                </div>
    ...
    <script>
        $(document).ready(function() {
            $(".owl-carousel").owlCarousel({
                ...
                responsive: {
                    0: {
                        items: 2 // Displays exactly 2 items on mobile screens
                    },
                    600: {
                        items: 3 // Displays 3 items on mid-range viewports
                    },
                    1000: {
                        items: 4 // Displays 4 items on desktop layouts
                    }
                }
            });
        });
